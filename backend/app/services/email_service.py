...
def send_report_email(email_config, excel_bytes, word_bytes, html_content):
    ...
    msg['Subject'] = f"商机雷达 - 抓取分析报告 {time.strftime('%Y-%m-%d')}"
    ...
    if excel_bytes:
        ...
        part_excel.add_header('Content-Disposition', 'attachment', filename="商机清单.xlsx")
        msg.attach(part_excel)

    # 附件 - Word
    if word_bytes:
        part_word = MIMEApplication(word_bytes)
        part_word.add_header('Content-Disposition', 'attachment', filename="商机分析报告.docx")
